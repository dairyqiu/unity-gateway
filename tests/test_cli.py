...
class TestConfigureAgentsSelection:
    ...
    def test_strict_error_mentions_skip_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(cli_mod, "_fetch_managed_config", lambda *a, **k: (None, False, False))
...
class TestConfigureProfilesFlag:
    ...
    def test_use_pat_is_forwarded(self):
        with (
            patch("ucode.cli.install_databricks_cli"),
            patch("ucode.cli.install_tool_binary"),
            patch("ucode.cli.list_profile_entries", return_value=self.PROFILE_ENTRIES),
...
class TestPostConfigureValidation:
    """Post-configure validation uses the managed-resolved config, not the stripped state."""
    ...
    def test_single_tool_configure_validates_with_managed_provider(self, monkeypatch):
            ...
            "enabled_agents": {
                "gemini": {
                    "model_config": {
                        ...
                        "model_services": ["managed-gemini-2.0-flash"],
                    }
                }
            }
        }
        ...
        # The validation path re-resolves the state with the managed config before validating.
        ...
        # validate_env should have seen the managed provider B, not the stripped developer provider A.
    ...
    def test_validate_tool_accepts_pre_resolved_state(self, monkeypatch):
        ...
        # Call validate_tool with an explicit pre-resolved state.
